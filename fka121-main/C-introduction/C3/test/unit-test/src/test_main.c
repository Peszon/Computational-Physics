 ...
 * cmocka which is a better designed
...
START_TEST(test_L2_Norm)
{
    ...
    // Calculate the addition
    double answer1 = L2_Norm(v1, ndims);
    double answer2 = L2_Norm(v2, ndims);

    // Now test! Is the calculated answer (test answer) equal to the known answer?
    ck_assert_double_eq_tol(answer1, sqrt(14.), 1e-10);
    ck_assert_double_eq_tol(answer2, sqrt(1694.), 1e-10);
}
