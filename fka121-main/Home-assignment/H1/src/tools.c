            ...
            int time_lag_ind)
{
    ...
    for (int i = 0; i < data_len; i++) {
        ...
        if (i + time_lag_ind < data_len) {
            sum_prod += data[i] * data[i + time_lag_ind];
        }
    }
...
double block_average(double *data,
    ...
    double block_average_arr[num_blocks];

    for (int i = 0; i < num_blocks; i++) {
        double block_sum = 0;
        for (int j = 0; j < block_size; j++) {
            block_sum += data[j + block_size * i];
        }
        block_average_arr[i] = block_sum / block_size; 
    }

    return block_size * pow(standard_deviation(block_average_arr, num_blocks),2) / pow(standard_deviation(data, data_len),2);
}